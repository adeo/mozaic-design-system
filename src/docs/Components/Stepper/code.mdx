...
```html
<nav class="mc-stepper">
  <ol class="mc-stepper__list">
    <li class="mc-stepper__item mc-stepper__item--current">
      <span class="mc-stepper__number">1</span>
    ...
    <li class="mc-stepper__item">
      <span class="mc-stepper__number">&nbsp;</span>
      <div class="mc-stepper__detail">
...
In its initial state, before the start of the user journey, the first step is by default always in the active/current state.

</Highlight>

<Highlight theme="warning">

A **Stepper** can have only one element that indicate the active/current state.
...
All steps **before** the active step are considered to be validated steps.

You can therefore apply to the `.mc-stepper__item` element of these validated steps, the modifier `mc-stepper__item--validate`.

In addition to adding the `mc-stepper__item--validate` modifier, you must also replace the `mc-stepper__number` element with an icon indicating that the step has been validated.

```html
<li class="mc-stepper__item mc-stepper__item--validate">
  <div class="mc-stepper__number">
...
**For this reason, we recommend only displaying the active step on these resolutions.**
...
Adding this modifier will keep only the active step visible on resolutions below the `M` breakpoint.
