...
## Basic usage
To create a basic accordion, wrap a `checkbox`, a `label` and a `div` inside a `div` and apply the following classes: 
...
You only need to add the `mc-accordion__picto` CSS class to you icon.
