...
<Preview path="with-attribute-accept" />
...
<Preview path="uploaded-files-with-errors" />
...
When using the fileuploader as part of a form, you must make sure to respect the following structure:
...
When the fileuploader is mandatory, you must specify it to the user. 
...
<Highlight theme="tips" title="Semantic rule">
  ...
  When the fileuploader is mandatory the `required` attribute must be added on the `mc-fileuploader__input` element
...
To indicate that your fileuploader is not valid, several steps are to be implemented:
