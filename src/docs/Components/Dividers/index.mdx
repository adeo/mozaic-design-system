---
title: 'Dividers'
order: 1
description: ''
status:
  sketch: 'bêta'
  scss: 'bêta'
---

> Dividers are thin lines used to insert breaks into your layouts and group similar content.

--Preview--

## Variations

There are four different variations depending on your page needs. Every color variation has a vertical or an horizontal version to suit your pages design.

### Lightest

This lightest version must be used on dark background only to be visible.

### Light

We provide a light version of the dividers but you must be aware that this version is not AA accessible. This version must be used for non mandatory dividers to understand the content of your page for high end screens only.

### Default

This is the version you should use in your layouts. This default version is AAA accessible and will be seable by every users.

### Dark

The dark version is very usefull when you need to insert a real break or a real distinction in your design to visually guide the eye of your user.

## Do's and Don'ts

<HintItem>

</HintItem>

<HintItem dont>

</HintItem>
