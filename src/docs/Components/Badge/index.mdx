...
<HintItem>
  Always use a label in adequation with the state to help the user to understand
  the status whatever the state's colour.
</HintItem>

<HintItem dont>
  Never use a badge as a category, it only must be used to display status.
</HintItem>
