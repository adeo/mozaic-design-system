...
In order to use the font proposed in Mozaic in your code, start by importing the `_all-settings.scss` file in the following way:
...
<Highlight theme="tips" title="Good to know">
  ...
  code when using the components and tools supplied by Mozaic.
</Highlight>
...
- `set-font-weight()`
- `set-font-face()`
- `set-font-scale()` _([see the dedicated documentation here](/Foundations/Typography/ScaleAndSizes/code/#the-font-scale-mixin-))_

## Details of the mixins

### The import-font-families() mixin
...
### The set-font-weight() mixin

The `set-font-weight()` mixin allows you to set the font weight you wish to use in your selector.
