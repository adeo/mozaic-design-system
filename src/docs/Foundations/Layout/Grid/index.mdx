...
> A grid is used to create a structured and harmonious design for every breakpoints. It is the frame of your page that allows you to build your layout by placing blocks of contents (texts, images, etc.) using a certain number of columns. Gutter are used to give some blank space between 2 separate blocks. A container is the combination of columns and gutters. The container can be full width (fluid) or have a fixed size (fixed container).
