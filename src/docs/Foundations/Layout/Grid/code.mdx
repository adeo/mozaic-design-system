...
> Flexy is a grid system and a general purpose layout tool based on Flex. It can be used to build page level layout as well as component level one. That's why it is not called "grid" but "flexy".
...
<HintItem>
  Use flexy when possible to create flex layouts inside smaller elements or components.
</HintItem>
...
When **used with gutters**, `.ml-flexy` uses the negative margin pattern : each column has a padding measuring half a gutter width, and the containing `.ml-flexy` has a negative margin equal to half a gutter width on both side as well.
...
Example : Create a blog post layout with an image and a text. On tablet screens, put the text in front of the image.
...
Please consider that you need to keep the import order clean and you should add your grid extension right after the "flexy" import.

We advise you to create a new scss file for your extensions:
