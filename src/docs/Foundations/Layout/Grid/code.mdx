...
#### Columns
`ml-flexy__col` defines individual columns inside the grid. With Flexbox columns widths are set in percentages relative to their parent element so columns without a spec’d width will automatically be equal. For example, four instances of `ml-flexy__col` will give you four equal width columns — each 25% of the parent from the small breakpoint on up.
