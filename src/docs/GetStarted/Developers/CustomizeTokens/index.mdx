...
As a reminder, we call tokens a series of JSON files containing mostly color values (among others things). Those files are transpiled into SCSS, js, ios, and android formats, to be used within multiple platforms and technologies.

<Highlight theme="tips" title="For more informations">
...
By doing that, you tell Mozaic where to look for the source tokens, and where to save the compiled ones. We also use the `localTokensExportPath` inside Postcss to tell it where to look for the SCSS tokens. By doing that, the SCSS will load the customized ones instead of the ones living in the npm package.
...
In this example, Mozaic will replace the `color.primary-01.100` and set it to `#000` (black), but all the other existing values for `color.primary-01.xxx` will be kept as defined in the npm package source. Meaning `color.primary-01.100` will be customized to `#000` (black), and `color.primary-01.200` will still be green.
...
You can see that we build variables but also a sass map. The map is useful when we need to loop through values to create multiple variations of a pattern. More informations about that in chapter 5.

</Highlight>

<Highlight theme="tips" title="Reminder">

All tokens are available through `@import 'settings-tools/_all-settings';`
...
You can look in the specs tab of the component documentation to help you define which tokens need to be overridden. We generally define most of the tokens used to theme our components.
