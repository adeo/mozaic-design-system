...
Styelint comes with its configuration in Mozaic and it is provided at compilation time. You don't necessarily need a root config file, but in some case you may want to have it, for exemple to configure prettier-stylelint on your IDE, or to use stylelint outside of the compilation pipeline.
...
### B) Create a styelint config file :
...
Run the `npm run css:lint-fix` again. Styelint fixed this outrageous mistake and added the line break after the mixin instantiation.
