...
If you [configured PostCSS properly](/GetStarted/Developers/configurePostCss/), you don't need to specify the full relative path to the node modules directory, you can import directly from the `@mozaic-ds/styles` directory :
...
If you can't do this, this mean that you don't use the Mozaic postcss config, which is **mandatory for Mozaic to work as expected**.
...
// Generics
@import 'generics/_g.***.scss';
@import 'generics/_g.***.scss';

// Elements
@import 'elements/_e.***.scss';
@import 'elements/_e.***.scss';
