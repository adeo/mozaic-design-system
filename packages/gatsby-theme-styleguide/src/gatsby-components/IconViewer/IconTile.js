  ...
  buildIconData(name, icons) {
    ...
    Object.keys(dataIcon.sizes).map((size) => {
      if(!icons[size]) {
        ...
        dataIcon.sizes[size].fullName = dataIcon.sizes[size].url.replace('/icons/', '').replace('.svg', '');
      }
  ...
  render() {
    return (
      <IconDetail>
        ...
        <IconWrapper><img src={`${this.state.allIcon[this.state.currentSize].url}`} width={this.state.currentSize} height={this.state.currentSize} alt=""/></IconWrapper>
        <SizesList>
          {Object.keys(this.dataIcon.sizes).map((size) => 
            this.state.allIcon[size].url ? (
              <SizeItemAvail key={`${this.props.name}-${size}`} isActive={this.state.currentSize === size}>
        ...
        <Copy value={this.state.allIcon[this.state.currentSize].fullName} children={<TechnicalName value={this.state.allIcon[this.state.currentSize].fullName}/>}/>
      </IconDetail>
    )
  }
}
...
const TechnicalName = ({value}) => {
  return (
    <IconSizeName>{value}</IconSizeName>
  );
}
