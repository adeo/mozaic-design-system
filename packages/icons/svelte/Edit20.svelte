<svelte:options tag={null} />
  <script>
  export let id = undefined;
  export let style = undefined;
  export let className = undefined;
  export let fill = undefined;
  export let size = "1.25rem";
</script>

<svg aria-hidden="true" width={size} height={size} id={id}  style={style} class={className} fill={fill} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
  <g clip-path="url(#a)">
    <path fill-rule="evenodd" d="M15.947 1.97a.75.75 0 0 0-1.06 0l-8.334 8.333a.75.75 0 0 0-.172.267l-1.25 3.333a.75.75 0 0 0 .966.966l3.333-1.25a.75.75 0 0 0 .267-.172l8.333-8.333a.75.75 0 0 0 0-1.061L15.947 1.97Zm-8.474 9.96-.358.955.955-.358-.597-.597Zm1.517.103 5.751-5.751-1.023-1.023-5.75 5.751 1.022 1.023Zm6.458-6.459-1.022-1.022.99-.991 1.023 1.022-.99.991ZM2.5 5.084a.75.75 0 0 0-.75.75V17.5c0 .414.336.75.75.75h11.667a.75.75 0 0 0 .75-.75v-3.333a.75.75 0 0 0-1.5 0v2.583H3.25V6.583h2.583a.75.75 0 1 0 0-1.5H2.5Z"/>
  </g>
  <defs>
    <clipPath id="a">
      <rect width="20" height="20" fill="#fff"/>
    </clipPath>
  </defs>
</svg>
