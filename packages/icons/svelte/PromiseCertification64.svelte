/*
DON'T EDIT THIS FILE
It was auto-generated by "iconCompiler" script
*/


<script>
  export let id = undefined;
  export let style = undefined;
  export let size = "4rem";
</script>

<svg width={size} height={size} fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><path d="M29.92 30.1a1.48 1.48 0 002.11 0l9-9a1.5 1.5 0 10-2.09-2.1l-8 7.94L27.06 23a1.5 1.5 0 10-2.12 2.12z"/><path d="M53.79 48.74l-7.31-12.47a19 19 0 10-29 0l-7.27 12.47a1.49 1.49 0 000 1.51 1.51 1.51 0 001.3.75h8.67l4.56 7.2a1.51 1.51 0 001.27.7 1.5 1.5 0 001.27-.76L32 49.85l4.69 8.29a1.5 1.5 0 001.31.76 1.51 1.51 0 001.27-.7l4.56-7.2h8.67a1.51 1.51 0 001.3-.75 1.49 1.49 0 00-.01-1.51zM32 8a16 16 0 11-16 16A16 16 0 0132 8zm-6.07 46.48l-3.66-5.78A1.51 1.51 0 0021 48h-6.88l5.59-9.54a18.85 18.85 0 008.15 4.07l2.42 4.27zM43 48a1.51 1.51 0 00-1.27.7l-3.66 5.78L31.56 43H32a18.87 18.87 0 0012.29-4.54L49.88 48z"/></svg>